    ...
    internal static class PoseNameAllocator
    {
            ...
            HashSet<string> usedLayerNames)
        {
            ...
            definition.LayerName = buildable
                ? NormalizeLayerName(group, definition.Id, usedLayerNames)
                : LayerBaseName(group);
            definition.ParameterName = buildable && PoseTuneCompilerRules.RequiresPoseSelectionParameter(root, definition)
                ? NormalizeGroupParameter(root, group, definition.Id, usedParameters)
                : GroupParameterName(root, group, definition.Id);
        }

        private static string NormalizeLayerName(PoseGroup group, string groupId, HashSet<string> usedLayerNames)
        {
            ...
            var suffix = PoseTuneNames.ShortId(groupId);
        ...
        private static string NormalizeGroupParameter(
            PoseTuneRoot root,
            PoseGroup group,
            string groupId,
            HashSet<string> usedParameters)
        {
            var baseName = GroupParameterName(root, group, groupId);
        ...
        private static string GroupParameterName(PoseTuneRoot root, PoseGroup group, string groupId)
        {
            ...
            {
                parameterSegment = "Custom_" + PoseTuneNames.ShortId(groupId);
            }
